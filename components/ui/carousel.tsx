...
const Carousel = memo(
  ({
    ...
    const rotation = useSpring( useTransform(scrollY, v=> v*.1))
    ...
    return (
      <div
        className="flex  h-full p-30 items-center justify-center   "
        style={{
          perspective: "1000px",
      ...
      >
        ...
        >
          {cards.map((imgUrl, i) => (
            <motion.div
              key={`key-${imgUrl}-${i}`}
              className="absolute flex h-full origin-center items-center justify-center rounded-xl bg-mauve-dark-2 p-4"
              style={{
                width: `${faceWidth}px`,
                transform: `rotateY(${
                  i * (360 / faceCount)
...
function ThreeDPhotoCarousel({className}: {
  className?: string
}) {
  ...
  return (
    <motion.div layout className={cn(
      ...
      <div className="relative h-[500px] max-w-4xl mx-auto  mx-auto bg-secondary/80 rounded-2xl overflow-hidden">
        <Carousel
